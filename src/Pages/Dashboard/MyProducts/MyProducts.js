...
const MyProducts = () => {
    ...
    const {data: products = [], refetch, isLoading} = useQuery({
        queryKey: ['products'],
        queryFn: async () =>{
            const res = await fetch(`https://car-seller-server.vercel.app/products/my-products?email=${user?.email}`, {
                headers: {
    ...
    const handleProductDelete = product =>{
        fetch(`https://car-seller-server.vercel.app/products/${product._id}` ,{
            method: 'DELETE',
        })
        .then(res => res.json())
        .then(data => {
