...
const AllSeller = () => {
    ...
    const { data: sellers = [], isLoading, refetch } = useQuery({
        queryKey: ['sellers'],
        queryFn: async () => {
            const res = await fetch('https://car-seller-server.vercel.app/users/seller?role=seller', {
                headers: {
    ...
    const handleUpdateSellerStatus = id => {
        fetch(`https://car-seller-server.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
    ...
    const handleSellerDelete = seller => {
        fetch(`https://car-seller-server.vercel.app/users/seller/${seller?._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
